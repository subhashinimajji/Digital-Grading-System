...
def calculate_accuracy(ngram, rouge_scores, similarity_score_1_2):

    # Calculate the accuracy score as a weighted sum
    accuracy = (
        (100 * ngram) +
        (100* rouge_scores )+
        (100* similarity_score_1_2)
    ) / 300
...
#         Spacy_sim_score = doc1.similarity(doc2)
...
#f1 score nd precison
    
        

 
    
    return render_template('result1.html', text1=text1, text2=text2,answer=stemmed_text, question=stemmed_text1 , rouge=rouge_scores, wob=similarity_score_1_2,similarity_b= bert_sc,keywords_score=answer_score,bleu_sc=bleu_score_pe) 
